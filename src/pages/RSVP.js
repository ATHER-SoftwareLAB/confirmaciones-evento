...
export default function RSVP() {
  ...
  const fechaEvento = evento?.evento_fecha ? new Date(evento.evento_fecha) : null
  const fechaLimite = evento?.evento_fecha_limite ? new Date(evento.evento_fecha_limite) : null
  const countdown = fechaEvento ? getCountdown(fechaEvento, now) : null
  ...
  return (
    <div style={S.page}>
      <div style={S.container}>
        ...
        <div style={S.card}>
          ...
          {fechaEvento && (
            <div style={S.dateBlock}>
              <div style={S.dateText}>{formatFecha(fechaEvento)}</div>
              {countdown && !countdown.pasado && (
                <div style={S.countdownRow}>
                  ...
                  <CountdownUnit value={countdown.seg} label="seg" />
                </div>
              )}
              {countdown?.pasado && <div style={S.pastBadge}>El evento ya tuvo lugar</div>}
            </div>
          )}
          ...
          {evento?.evento_ubicacion_nombre && (
            <div style={S.locationBlock}>
              ...
              <div>
                <div style={S.locationName}>{evento.evento_ubicacion_nombre}</div>
                {evento.evento_maps_url && (
                  <a href={evento.evento_maps_url} target="_blank" rel="noreferrer" style={S.mapsLink}>
          ...
          {respondido ? (
            ...
            <div style={S.actionBlock}>
              {fechaLimite && countdownLimite && !countdownLimite.pasado && (
                <div style={S.deadlineNote}>
                  Confirma antes del {formatFecha(fechaLimite)}
                </div>
              )}
        ...
        <div style={S.footer}>Con cariño esperamos contar contigo</div>
      </div>
    </div>
  )
}
...
const S = {
  ...
  loadingDot: { width: 12, height: 12, borderRadius: '50%', background: '#c9a96e', animation: 'pulse 1.2s ease-in-out infinite' },
  ...
  imageWrap: { borderRadius: 20, overflow: 'hidden', marginBottom: -40, position: 'relative', zIndex: 1, boxShadow: '0 12px 40px rgba(0,0,0,0.5)' },
  image: { width: '100%', display: 'block', maxHeight: 320, objectFit: 'cover' },
  card: {
  ...
  title: { fontFamily: "'Playfair Display', serif", fontSize: 28, textAlign: 'center', lineHeight: 1.3, marginBottom: 24, color: '#f0e8d8' },
  dateBlock: { textAlign: 'center', marginBottom: 24 },
  dateText: { fontSize: 14, color: '#c9a96e', marginBottom: 16, textTransform: 'capitalize' },
  countdownRow: { display: 'flex', justifyContent: 'center', gap: 10 },
  ...
  pastBadge: { fontSize: 13, color: '#9990b8' },
  locationBlock: { display: 'flex', gap: 12, alignItems: 'flex-start', background: '#22223b', borderRadius: 12, padding: 16, marginBottom: 24 },
  locationIcon: { fontSize: 18 },
  locationName: { fontSize: 14, fontWeight: 500, marginBottom: 4 },
  mapsLink: { fontSize: 12, color: '#c9a96e', textDecoration: 'none' },
  ...
  deadlineNote: { fontSize: 12, color: '#9990b8', marginBottom: 16 },
  ...
  btnYes: {
    ...
    color: '#0f0f1a', fontWeight: 600, fontSize: 14, cursor: 'pointer', transition: 'background 0.2s',
  },
  btnNo: {
